...
```md
src/
├── App.tsx                    # 主应用 - 核心逻辑和状态管理
├── components/
│   ├── Header.tsx            # 顶部导航栏 - 用户信息、周数、日历（与Edit Zone对齐）
│   ├── Sidebar.tsx           # 左侧边栏 - 日记区域
│   ├── CoursesAndTasks.tsx   # 并排组件 - 课程列表 + 任务卡片
│   ├── Timetable.tsx         # 时间表网格 - 课程背景块 + 任务圆角矩形
│   ├── EditZone.tsx          # 右侧编辑面板 - 编辑表单
...
| 组件 | 职责 | 新特性 |
| ------ | ------ | ------ |
| **Header** | 顶部导航、学校信息、用户信息、周数显示、日历（与Edit Zone垂直对齐） | 用户头像、日历弹层、金色周标题 |
| **Sidebar** | 学习日记区域 | 简化设计，齐平时间表 |
| **CoursesAndTasks** ⭐ | 并排显示课程列表（左）+ 任务卡片（右） | 新组件，共享一个区域，无分界线 |
| **Timetable** ⭐ | 时间网格、课程背景块、任务圆角矩形 | 课程填充背景，任务浮层显示 |
| **EditZone** | 任务单元编辑、课程编辑、任务编辑 | 简洁编辑面板，日历移至Header |
...
- **左侧**：学校名称（"XXX Univ." 红色）+ 用户头像（48x48px 蓝色圆形）+ 用户信息（"Greatest Me" + 邮箱）+ 登出按钮
- **中间**：Week N（32px 金色#c9a961 标题）+ 日期范围（yyyy.mm.dd ~ yyyy.mm.dd）
- **右侧**：日期显示 + Calendar 按钮（📅 日历），点击后在按钮下方显示日历弹层
  - 日历弹层：绝对定位在按钮下方，与 Edit Zone 垂直对齐
